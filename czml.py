	...
	def data(self):
		...
		for attr in self.properties:
			...
			if a is not None:
				...
				if isinstance(a, (_CZMLBaseObject, _Colors, _Coordinates, _Positions)):
					d[attr] = a.data()
				else:
					d[attr] = a
		return d
...
class Point(_CZMLBaseObject):
	...
	def __init__(self, show=False, color=None, pixelSize=None, outlineColor=None, outlineWidth=None):
		self.show = show
		self.color = color
...
class Material(_CZMLBaseObject):
	...
	grid = class_property(Grid, 'grid', doc="""Fills the surface with a grid.""")
	image = class_property(Image, 'image', doc="""The image to display on the surface.""")
	stripe = class_property(Stripe, 'stripe', doc="""Fills the surface with alternating colors.""")
	solidColor = class_property(SolidColor, 'solidColor',
	                            doc="""Fills the surface with a solid color, which may be translucent.""")
	polylineGlow = class_property(PolylineGlow, 'polylineGlow', doc="""Colors the line with a glowing color.""")
...
class Path(_DateTimeAware, _CZMLBaseObject):
	...
	_properties = ('show', 'width', 'leadTime', 'trailTime', 'resolution', 'material', 'position')
...
class Polygon(_DateTimeAware, _CZMLBaseObject):
	...
	_properties = ('show', 'fill', 'height', 'outline', 'stRotation', 'granularity', 'extrudedHeight',
	               'perPositionHeight', 'outlineColor', 'material', 'positions')
...
class CZMLPacket(_CZMLBaseObject):
	...
	_properties = (
		'id', 'description', 'version', 'availability', 'billboard', 'clock', 'position', 'label', 'point', 'positions',
		'polyline', 'polygon', 'path', 'orientation', 'ellipse', 'ellipsoid', 'cone', 'pyramid')
