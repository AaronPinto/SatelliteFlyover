...
import math, pkg_resources, pytz
...
def create_czml_file(start_time, end_time):
	...
	packet.clock = {"interval": interval, "currentTime": start_time.isoformat(), "multiplier": MULTIPLIER,
...
def get_future_sat_positions(sat_tle, number_of_positions, start_time):
	...
	for i in range(0, number_of_positions):
		...
		eci_position, eci_velocity = sat_tle.propagate(current_time.year, current_time.month, current_time.day,
...
def get_satellite_orbit(raw_tle, sim_start_time, sim_end_time, czml_file_name):
	...
	if DEBUGGING:
		print()
		print('Satellite Name: ', sat.sat_name)
		print('TLE Epoch: ', sat.tle_epoch)
		print('Orbit time in Minutes: ', sat.orbital_time_in_minutes)
		print()

	sat_packet = create_satellite_packet(sat.sat_name, sat.tle_object, sat.orbital_time_in_minutes, sim_start_time,
	                                     sim_end_time, sat.rgba)
	...
	if time_list is not None and len(time_list) > 1:
		for i in range(len(time_list) - 1):
			for sat in satellite_array:
				...
				sat_packet = create_satellite_packet(sat.sat_name, sat.tle_object, sat.orbital_time_in_minutes,
				                                     time_list[i], time_list[i + 1], sat.rgba)
				doc.packets.append(sat_packet)
	else:
		for sat in satellite_array:
			sat_name = sat.sat_name
			...
			sat_packet = create_satellite_packet(sat.sat_name, sat.tle_object, sat.orbital_time_in_minutes, start_time,
			                                     end_time, sat.rgba)
			doc.packets.append(sat_packet)
